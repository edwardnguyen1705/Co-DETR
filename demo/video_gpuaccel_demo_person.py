...
def main():
    ...
    video_writer = None
    if args.out:
        video_writer = ffmpegcv.VideoWriterNV(args.out, 'h264', fps=video_origin.fps)

    zeros_a = np.zeros((1, 5), dtype=np.float32)
    new_ret = [zeros_a for i in range(80)]
    ...
    with torch.no_grad():
        for frame_resize, frame_origin in zip(
            ...
            frame_mask = model.show_result(
                frame_origin, new_ret, score_thr=args.score_thr)
            if args.show:
